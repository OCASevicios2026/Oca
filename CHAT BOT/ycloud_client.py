...
async def download_media(link: str) -> bytes:
    """Descarga el archivo de un mensaje multimedia entrante (imagen, audio, PDF).

    YCloud incluye un ``link`` firmado en el webhook (image.link, audio.link,
    document.link). Para descargarlo hay que enviar el header ``X-API-Key``.
    """
    async with httpx.AsyncClient(timeout=60) as client:
        file_resp = await client.get(
            link, headers={"X-API-Key": settings.ycloud_api_key}
        )
        if file_resp.status_code >= 400:
            raise RuntimeError(f"YCloud media download error {file_resp.status_code}: {file_resp.text}")
