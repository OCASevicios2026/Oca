...
**Gemini 3.5 Flash** para conversaciones naturales, análisis de imágenes, notas de
voz y documentos PDF. Registra leads de cotización en **PostgreSQL**.

- Mensajería WhatsApp: [YCloud](https://www.ycloud.com) (WhatsApp Business API, Meta BSP)
- IA: [Gemini 3.5 Flash](https://ai.google.dev/gemini-api/docs) (Google AI Studio)
...
Usuario WhatsApp  →  YCloud (webhook)  →  /webhook (FastAPI)  →  chatbot.py (reglas)
                                                                      │
                                    si no entiende / multimedia → Gemini 3.5 Flash
                                                                      │
...
1. Si es **multimedia** (imagen, nota de voz o PDF) se descarga y se envía a Gemini
   para su análisis.
2. Si es **texto**, primero se evalúa el flujo por reglas (menú, servicios,
   ...
   respuesta a Gemini con el historial de la conversación (memoria por usuario).
...
├── prompts.py         # Prompt de sistema de Gemini + formatos
├── gemini.py          # Cliente de Gemini (texto, imagen, audio, PDF)
...
2. Agrega `GEMINI_API_KEY` y `GEMINI_MODEL=gemini-3.5-flash` a tu `.env` y a Railway.
...
- *Cualquier otra cosa*: Gemini responde con el historial de la conversación.
- *Imagen / nota de voz / PDF*: se descarga el archivo y Gemini lo analiza.
